...
import { chapterAndTopicTable, courseAndChapterTable, getParentChildCategories } from "../../../Redux/Actions/Editor/Category";
...
const ChapterAndTopics = () => {
  ...
  const ParentChildCategories = async () => {
    const responseTwo = await dispatch(chapterAndTopicTable(role, token));
    setParentChidCategory(responseTwo);
  };
          ...
          <TableBody>
            <DragDropContext onDragEnd={handleDragEnd}>
              {parentChidCategory?.map((row, index) => (
                <>
                  <Droppable droppableId={row.name}>
                    {(provider) => (
                      <TableRow
                        hover
